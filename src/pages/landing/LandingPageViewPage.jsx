...
const DEVICE_ID_KEY = "kaflifestyle_device_id";
...
export default function LandingPageViewPage({ campaign }) {
  ...
  const productOptions = useMemo(
    () =>
      getLandingProductOptions(campaign, {
        productName,
        price,
        originalPrice,
        bannerImage,
  ...
  useEffect(() => {
    ...
    const commonData = {
      ...
      content_name: title,
      contents: contentId
        ? [{ content_id: contentId, quantity: 1, price }]
        : [],
      value: price,
      currency: "BDT",
    };
  ...
  function buildLandingOrderPayload({ status = "pending", phoneNumber } = {}) {
    ...
    const normalizedPhone =
      phoneNumber || normalizeBangladeshPhone(form.phone) || form.phone.trim();
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(async () => {
      try {
        ...
        if (draftId) {
          ...
          if (leadTrackedOrderIdRef.current !== draftId) {
            ...
            void trackMarketingEvent("Lead", {
              ...
              customData: {
                content_ids: getSelectedOrderItems()
        ...
        console.warn(
          "Incomplete landing order save failed:",
          error?.message || error,
        );
      }
    }, 700);
  ...
  async function handlePlaceOrder() {
    ...
    const contentId = String(
      selectedItems[0]?.productId || campaign?.productId || campaign?.Id || "",
    );
    const trackingData = {
    ...
    try {
      const payload = buildLandingOrderPayload({
        phoneNumber: normalizedPhone,
      });
      ...
      void trackMarketingEvent("Purchase", {
        userData: {
          ...trackingUser,
          customerId:
            placedOrderData.customerId || placedOrderData.customer?.Id,
        },
        customData: {
          ...trackingData,
  ...
  return (
    <div className="min-h-screen bg-white text-slate-900">
      ...
      <section className="bg-[#e4f3df] px-4 pb-14 pt-8 text-center">
        ...
        <p
          className="mt-4 text-xl font-black md:text-3xl"
      ...
      <section className="px-4 py-16 text-center">
        <div className="mx-auto max-w-5xl">
          ...
          <p
            className="mx-auto mt-6 max-w-4xl text-base leading-8"
            style={{ color: colors.fontColor }}
          >
              ...
              {featureImages.map((item, index) => (
                ...
                >
                  {item.imageUrl ? (
                    ...
                    <div className="grid h-36 place-items-center bg-green-50 text-3xl">
                      ✅
                    </div>
                  )}
                  <p
      ...
      >
        <div className="mx-auto max-w-5xl text-center">
          ...
          <div
            className="mx-auto mt-6 max-w-4xl text-left text-base font-semibold leading-8"
            style={{ color: colors.fontColor }}
          >
            {whyChooseUs ? (
              <p>{whyChooseUs}</p>
              ...
              <p>
                ✅ ১০০% অরিজিনাল পণ্য ✅ দ্রুত ডেলিভারি ✅ ক্যাশ অন ডেলিভারি
                সুবিধা ✅ সহজ অর্ডার প্রক্রিয়া
              </p>
            )}
          </div>
          <a
            ...
            <div>
              ...
              <div className="mt-4 overflow-hidden rounded border border-slate-200">
                {SHIPPING_OPTIONS.map((option) => (
                  ...
                  >
                    <span
                      className={`h-5 w-5 rounded-full border ${form.shipping === option.id ? "border-green-600 bg-green-600" : "border-slate-300"}`}
                    />
                    {option.label === "Inside Dhaka"
                      ? "ঢাকার ভিতরে ৮০ টাকা"
                      : option.label === "Outside Dhaka"
                        ? "ঢাকার বাইরে ১৩০ টাকা"
                        : `${option.label} ${option.charge} টাকা`}
                  </button>
                ))}
              </div>
              {orderError ? (
                    ...
                    <tr key={item.id} className="bg-slate-200">
                      <td className="p-3">
                        <div className="flex items-center gap-2">
                          <img
                            src={imageUrl(item.image) || bannerImage}
                            alt={item.name}
...
function RegularOrderInput({ icon, placeholder, value, onChange }) {
  return (
    <label className="mb-3 flex items-center overflow-hidden rounded border border-slate-200 bg-white">
      <span className="flex h-11 w-12 items-center justify-center bg-slate-100 text-slate-600">
        {icon}
      </span>
      <input
        value={value}
      ...
      >
        ...
        <main className="mx-auto max-w-[1340px] px-4 pb-12">
          ...
          <Notice tone="yellow">
            ...
            <br />
            <span className="text-emerald-700">
              ক্যাম্পেইন অফার চলে যাওয়ার আগেই অর্ডার করুন
            </span>
          </Notice>
          <Notice tone="purple">
          ...
          {relatedProducts.length ? (
            <section className="mt-10 text-center">
              ...
              <p className="mt-1 text-xs text-slate-500">
                More great products from the same category
              </p>
...
function ProductOptionsCheckout({
  ...
  return (
    <div
      className={`rounded border border-blue-200 bg-blue-50 ${compact ? "p-4" : "p-0 border-0 bg-transparent"}`}
    >
              ...
              <div className="min-w-0 flex-1">
                ...
                <div className="mt-1 flex flex-wrap items-center gap-2">
                  <div className="inline-flex items-center rounded border border-slate-200 bg-slate-50 text-xs font-bold">
                    ...
                    <span className="min-w-7 px-2 py-1 text-center">
                      {cartItem?.qty || 1}
                    </span>
                    <button
                      type="button"
  ...
  const defaultQuickLinks = [
    {
      ...
      href: supportPhone
        ...
        : "#",
      onClick: () =>
        trackLandingContactClick("Landing footer support", supportPhone),
    },
