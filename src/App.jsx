...
const DASHBOARD_NAV_STORAGE_KEY = "kaflifestyle-dashboard:navigation";
...
function normalizeBanner(item) {
  ...
  return {
    ...
    imageColor: item.imageColor ?? "linear-gradient(135deg, #c5bfb4, #33302c)",
    status: item.status === true || item.status === "Active",
    sortOrder: item.sortOrder ?? null,
  };
}
...
function App() {
  ...
  const [blogs, setBlogs] = useState([
    {
      ...
      imageColor: "linear-gradient(135deg, #f8f7f4, #4d4943)",
      description: "",
      status: false,
    },
    {
      ...
      imageName: "",
      imageText: "Kaf Lifestyle BD",
      imageColor: "linear-gradient(135deg, #070707, #6f6a63)",
      description: "",
      status: false,
    },
  ]);
  ...
  const [bannerCategories, setBannerCategories] = useState([
    ...
    { id: 2, name: "Welcome to Kaf Lifestyle BD", status: true },
  ]);
  ...
  const permissionSet = useMemo(() => getPermissionSet(user), [user?.Id, user?.role, JSON.stringify(user?.menuPermissions || [])]);
  ...
  function renderMain() {
    if (!isNavigationAllowed(currentNavigationState, permissionSet)) {
      const next = normalizeNavigationForPermissions(currentNavigationState, permissionSet);
      const hasAllowedFallback = Object.keys(next).some((key) => next[key] !== currentNavigationState[key])
        && isNavigationAllowed({ ...currentNavigationState, ...next }, permissionSet);

      if (hasAllowedFallback) {
        return (
          <div className="flex h-full items-center justify-center p-6">
            <div className="rounded-lg border border-gray-200 bg-white p-6 text-center shadow-sm">
              <h1 className="text-lg font-bold text-gray-800">Loading permissions...</h1>
            </div>
          </div>
        );
      }
