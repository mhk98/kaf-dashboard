...
export default function DateFilterPanel({ onFilterChange, filteredData, loading }) {
  ...
  const filterStats = [
    { label: 'Total Visitors', value: fmt(summary.totalVisitors), tone: 'border-blue-100 bg-blue-50 text-blue-700' },
    { label: 'Total Customers', value: fmt(summary.totalCustomers), tone: 'border-emerald-100 bg-emerald-50 text-emerald-700' },
  ];
      ...
      <div className="bg-white rounded-xl shadow overflow-hidden">
        ...
        <div className="p-4 space-y-3">
          ...
          <div className="grid grid-cols-2 gap-2">
            {filterStats.map((s, i) => (
              <div key={i} className={`rounded-lg border p-3 ${s.tone} ${loading ? 'animate-pulse' : ''}`}>
